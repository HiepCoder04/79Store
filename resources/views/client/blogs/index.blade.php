...
<style>
    ...
    .top-breadcrumb-area h2 {
        ...
        z-index: 2;
        font-size: 48px;
        color: #ffffff; /* Đảm bảo màu trắng cho tiêu đề */
        text-shadow: 2px 2px 4px rgba(0,0,0,0.8); /* Tăng đậm bóng đổ */
        letter-spacing: 2px;
        margin-bottom: 15px; /* Tăng khoảng cách với subtitle */
        font-weight: 700; /* Làm đậm chữ hơn */
    }
    
    .top-breadcrumb-area .subtitle {
        color: #ffffff; /* Đảm bảo màu trắng cho subtitle */
        ...
        padding: 0 15px; /* Thêm đệm hai bên */
            ...
            <div class="subtitle-wrapper">
                <p class="subtitle">
                    @if(isset($category))
                        {{ $category->name }}
                    @else
                        Khám phá kiến thức & cập nhật tin tức mới nhất
                    @endif
                </p>
            </div>
        </div>
                        ...
                        <ol class="popular-tags d-flex flex-wrap">
                            @if(isset($categories) && $categories->count() > 0)
                                @foreach($categories as $category)
                                    <li><a href="{{ route('client.blogs.category', ['slug' => $category->slug]) }}">{{ $category->name }} ({{ $category->blogs_count }})</a></li>
                                @endforeach
                            @else
                            ...
                            @foreach($featured_products as $product)
                                ...
                                <div class="single-best-seller-product d-flex align-items-center">
                                    <div class="product-thumbnail">
                                        @if($product->image)
                                            <a href="{{ route('client.products.show', $product->slug) }}"><img src="{{ asset($product->image) }}" alt="{{ $product->name }}"></a>
                                        @else
                                            <a href="{{ route('client.products.show', $product->slug) }}"><img src="{{ asset('client/img/bg-img/4.jpg') }}" alt="{{ $product->name }}"></a>
                                        @endif
                                    </div>

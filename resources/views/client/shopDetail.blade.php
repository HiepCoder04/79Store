    ...
    <div class="container">
        <div class="row align-items-center">
            ...
            <div class="col-12 col-md-6">
                <img class="d-block w-100" src="{{ asset(ltrim($product->galleries->first()->image ?? 'assets/img/bg-img/default.jpg', '/')) }}" alt="{{ $product->name }}">
            </div>

            <!-- Product Info -->
            <div class="col-12 col-md-6">
                <h4 class="product-title mb-2 text-uppercase">{{ $product->name }}</h4>
                <h4 class="text-success mb-3">{{ number_format($product->variants->first()->price ?? 0, 2) }}$</h4>
                ...
                <form method="POST" action="{{ route('cart.add') }}" class="mb-4">
                    ...
                    <div class="d-flex align-items-center mt-3 mb-3">
                        <div class="quantity">
                            ...
                            <input type="number" id="quantity" name="quantity" value="1" min="1" max="12" class="qty-text mx-2" style="width: 60px;">
                            <span class="qty-plus" onclick="document.getElementById('quantity').stepUp();"><i class="fa fa-plus"></i></span>
                        </div>
                        <button type="submit" class="btn alazea-btn ml-3">ADD TO CART</button>
                    </div>
                </form>
